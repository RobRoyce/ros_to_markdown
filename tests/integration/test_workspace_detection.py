...
def test_workspace_contains_packages():
    ...
    assert verify_package_exists(test_pkg), \
        f"test_pkg not found or invalid at {test_pkg}"
...
def test_ros_detector_live_environment(ensure_workspace):
    ...
    assert detected_distro == distro, \
        f"Expected distro {distro}, got {detected_distro}"
...
def test_ros_detector_workspace_tools(ensure_workspace):
    ...
    if ros_version == ROSVersion.ROS1:
        ...
        assert os.path.exists(os.path.join(workspace, "install/setup.bash")), \
...
def test_ros_detector_package_inspection(ensure_workspace):
    ...
    with open(package_xml_path) as f:
        content = f.read()
        if ros_version == ROSVersion.ROS1:
            assert 'format="2"' in content or 'format="1"' in content, \
                "ROS1 package.xml should use format 1 or 2"
        else:
            assert 'format="3"' in content, \
                "ROS2 package.xml should use format 3"
...
def test_ros_detector_python_path():
    ...
    if ros_version == ROSVersion.ROS1:
        ...
        assert "/opt/ros/noetic/lib/python3/dist-packages" in python_path, \
        ...
        if distro in ["rolling", "jazzy"]:
            ...
            assert expected_path in python_path, \
                f"ROS2 Python path for {distro} not properly set. Expected {expected_path} in {python_path}"