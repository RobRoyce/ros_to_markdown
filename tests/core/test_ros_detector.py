...
class TestROSDetectorWorkspace:
    ...
    def test_ros2_workspace_detection(self, mock_ros2_workspace):
        ...
        with patch('subprocess.run') as mock_run, \
             patch('ros_to_markdown.core.ros_detector.import_module') as mock_import:
            mock_run.return_value = MagicMock(returncode=0)
    ...
    @pytest.mark.parametrize("ros_distro,expected_dirs", [
    ...
    @pytest.mark.parametrize("command,expected_returncode", [
        (['catkin', '--version'], 0),
        (['colcon', '--version'], 0),
        (['invalid-command'], 1),
    ])
